import { FalsePositionInput, FalsePositionResult } from '../types/numerical';
import { formatOctaveNumber, formatOctaveExpression } from './common';

export function generateFalsePositionOctaveCode(input: FalsePositionInput, result: FalsePositionResult): string {
    let code = `% Método de Regla Falsa\n`;
    ...
    result.iterations.forEach((it) => {
        ...
        code += `w${k} = b${k} - ((feval(f,b${k})*b${k} - feval(f,b${k})*a${k})/(feval(f,b${k})-feval(f,a${k})));\n`;
        code += `fw${k} = feval(f, w${k});\n\n`;
    });
